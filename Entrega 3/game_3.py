...
################################################################
######## Larry Perez 10-10547 --- ENTREGA 3 TOPICOS IA #########
################################################################
...
def main(pc,itemlist,npcs):
    ...
    #for npc in npcs:
        #npc.nodeat = gamegraph.inwhatnode2d(npc.p)
    ...
    def sik(dat_npc):
        if (pc.nodeat != None) and (gamegraph.inwhatnode2d(pc.p) != None):
            if pc.nodeat.name != gamegraph.inwhatnode2d(pc.p).name:
                for record in dat_npc.path:
                    if record.connection != None:
                        record.connection.minusw()
                dat_npc.path = dat_npc.seekInGraphPW(gamegraph,pc.p)
    ...
    def toinitial(not_pc):
        return not pcingraph(not_pc)

    def toseek(not_pc):
        return pcingraph(not_pc)
    ...
    for i in range(0,len(npcs)-1):
        ...
        seeker[i] = state(sik,None,None)

        #### Moverse a uno de los nodos cercanos ####

        initial[i] = state(ini,None,halt)
                    
        transitions = [transition(toinitial,initial[i],seeker[i]),transition(toseek,seeker[i],initial[i])]
        
        q = statemachine(list([seeker[i],initial[i]]),seeker[i],npcs[i], transitions)
        maquinas.append(q)
    ...
    for npc in npcs:
        npc.path = npc.seekInGraphPW(gamegraph,pc.p)
...
npc3 = Kinematic([0,0,0],list([0,0,0]),2,0,0,0.025)
######################################
...
main(player,obstacles,[npc1,npc2,npc3])
